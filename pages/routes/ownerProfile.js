...
router.get('/me/profile', jwtMiddleware, async (req, res) => {
  try {
    console.log('GET /owners/me/profile - incoming Authorization header:', req.headers.authorization);
    console.log('GET /owners/me/profile - jwtMiddleware attached:', { user: req.user, userId: req.userId, token: req.jwtToken });

    const userPayload = req.user;
    if (!userPayload || userPayload.role !== 'owner') {
      console.log('GET /owners/me/profile - unauthorized (missing payload or role)');
      return res.status(401).json({ error: 'Unauthorized' });
    }

    const userId = req.userId;
    console.log('GET /owners/me/profile - resolved userId:', userId, 'typeof:', typeof userId);
    if (!userId) {
      console.log('GET /owners/me/profile - user id missing in token payload');
    ...
    console.log('GET /owners/me/profile - knex query fields:', fields, 'where id =', userId);
    const user = await knex('users').select(fields).where({ id: userId }).first();
    console.log('GET /owners/me/profile - DB returned user:', user);

    if (!user) {
      console.log('GET /owners/me/profile - user not found for id:', userId);
      return res.status(404).json({ error: 'User not found' });
    }

    return res.json({ user });
  } catch (err) {
    console.error('GET /owners/me/profile error:', err);
